...
> **正式提交说明（2026-07-30）：**本文负责逐项审计 issue #121 明列的候选方向，
> 不是全项目最终总账。跨分支最终状态与后来出现的 local
> orthogonal-contraction `active-qnc-candidate` 请见
> [完整挑战报告](../CHALLENGE_REPORT.md)；下列候选方向的正负结论和覆盖边界仍有效。
